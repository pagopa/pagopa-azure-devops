...
locals {
  ...
  # global secrets
  paywallet_iac_variables_secret = {}

  # code_review vars
  paywallet_iac_variables_code_review = {}
  # code_review secrets
  paywallet_iac_variables_secret_code_review = {}

  # deploy vars
  paywallet_iac_variables_deploy = {}
  # deploy secrets
  paywallet_iac_variables_secret_deploy = {}
}

module "paywallet_iac_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v7.0.0"
  count  = var.paywallet_iac.pipeline.enable_code_review == true ? 1 : 0
  path   = var.paywallet_iac.pipeline.path

  project_id                   = azuredevops_project.project.id
  repository                   = var.paywallet_iac.repository
  github_service_connection_id = azuredevops_serviceendpoint_github.azure-devops-github-pr.id

  pipeline_name_prefix = var.paywallet_iac.pipeline.pipeline_name_prefix



  variables = merge(
    local.paywallet_iac_variables,
    local.paywallet_iac_variables_code_review,
  )

  variables_secret = merge(
    local.paywallet_iac_variables_secret,
    local.paywallet_iac_variables_secret_code_review,
  )
...
module "paywallet_iac_deploy" {
  ...
  variables = merge(
    local.paywallet_iac_variables,
    local.paywallet_iac_variables_deploy,
  )

  variables_secret = merge(
    local.paywallet_iac_variables_secret,
    local.paywallet_iac_variables_secret_deploy,
  )
