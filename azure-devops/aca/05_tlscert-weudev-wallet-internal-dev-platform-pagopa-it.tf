variable "tlscert-weudev-aca-internal-dev-platform-pagopa-it" {
  default = {
    repository = {
      organization   = "pagopa"
...
locals {
  tlscert-weudev-aca-internal-dev-platform-pagopa-it = {
    tenant_id         = module.secrets.values["TENANTID"].value
    subscription_name = "DEV-PAGOPA"
    subscription_id   = module.secrets.values["DEV-SUBSCRIPTION-ID"].value
  }
  tlscert-weudev-aca-internal-dev-platform-pagopa-it-variables = {
    KEY_VAULT_SERVICE_CONNECTION = module.DEV-ACA-TLS-CERT-SERVICE-CONN.service_endpoint_name
  }
  tlscert-weudev-aca-internal-dev-platform-pagopa-it-variables_secret = {
  }
}

module "tlscert-weudev-aca-internal-dev-platform-pagopa-it-cert_az" {
  ...
  name       = "${var.tlscert-weudev-aca-internal-dev-platform-pagopa-it.pipeline.dns_record_name}.${var.tlscert-weudev-aca-internal-dev-platform-pagopa-it.pipeline.dns_zone_name}"
  #tfsec:ignore:GEN003
  ...
  variables = merge(
    ...
    local.tlscert-weudev-aca-internal-dev-platform-pagopa-it-variables,
  )

  variables_secret = merge(
    var.tlscert-weudev-aca-internal-dev-platform-pagopa-it.pipeline.variables_secret,
    local.tlscert-weudev-aca-internal-dev-platform-pagopa-it-variables_secret,
  )

  service_connection_ids_authorization = [
    module.DEV-ACA-TLS-CERT-SERVICE-CONN.service_endpoint_id,
  ]
