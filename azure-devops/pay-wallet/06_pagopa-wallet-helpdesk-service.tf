variable "pagopa-payment-wallet-helpdesk-service" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "pagopa-payment-wallet-helpdesk-service"
      branch_name     = "refs/heads/main"
      pipelines_path  = ".devops"
      yml_prefix_name = null
    }
    pipeline = {
      enable_code_review = false #project POC, no CR pipeline present right now
      enable_deploy      = true
      sonarcloud = {
        # TODO azure devops terraform provider does not support SonarCloud service endpoint
        service_connection = "SONARCLOUD-SERVICE-CONN"
        org                = "pagopa"
        project_key        = "pagopa_pagopa-payment-wallet-helpdesk-service"
        project_name       = "pagopa-payment-wallet-helpdesk-service"
      }
    }
  }
}
...
locals {
  # global vars
  pagopa-payment-wallet-helpdesk-service-variables = {
    cache_version_id = "v1"
    default_branch   = var.pagopa-payment-wallet-helpdesk-service.repository.branch_name
  }
  # global secrets
  pagopa-payment-wallet-helpdesk-service-variables_secret = {

  }
  # code_review vars
  pagopa-payment-wallet-helpdesk-service-variables_code_review = {
    danger_github_api_token = "skip"
    sonarcloud_service_conn = var.pagopa-payment-wallet-helpdesk-service.pipeline.sonarcloud.service_connection
 ...
 pagopa-payment-wallet-helpdesk-service-variables_secret_code_review = {
  ...
  # deploy vars
  pagopa-payment-wallet-helpdesk-service-variables_deploy = {
  ...
  # deploy secrets
  pagopa-payment-wallet-helpdesk-service-variables_secret_deploy = {
...
module "pagopa-payment-wallet-helpdesk-service_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v5.1.1"
  count  = var.pagopa-payment-wallet-helpdesk-service.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = data.azuredevops_project.project.id
  repository                   = var.pagopa-payment-wallet-helpdesk-service.repository
  github_service_connection_id = data.azuredevops_serviceendpoint_github.github_pr.service_endpoint_id
  path                         = "${local.domain}\\pagopa-payment-wallet-helpdesk-service"

  variables = merge(
    local.pagopa-payment-wallet-helpdesk-service-variables,
    local.pagopa-payment-wallet-helpdesk-service-variables_code_review,
  )

  variables_secret = merge(
    local.pagopa-payment-wallet-helpdesk-service-variables_secret,
    local.pagopa-payment-wallet-helpdesk-service-variables_secret_code_review,
  )
...
module "pagopa-payment-wallet-helpdesk-service_deploy" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_deploy?ref=v4.2.1"
  count  = var.pagopa-payment-wallet-helpdesk-service.pipeline.enable_deploy == true ? 1 : 0
  ...
  variables = merge(
    local.pagopa-payment-wallet-helpdesk-service-variables,
    local.pagopa-payment-wallet-helpdesk-service-variables_deploy,
  )

  variables_secret = merge(
    local.pagopa-payment-wallet-helpdesk-service-variables_secret,
    local.pagopa-payment-wallet-helpdesk-service-variables_secret_deploy,
  )
