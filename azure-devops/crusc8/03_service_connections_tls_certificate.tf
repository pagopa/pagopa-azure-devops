...
# DEV
#
module "DEV-CRUSC8-TLS-CERT-SERVICE-CONN" {
...
resource "azurerm_key_vault_access_policy" "DEV-CRUSC8-TLS-CERT-SERVICE-CONN_kv_access_policy" {
  provider     = azurerm.dev
  key_vault_id = data.azurerm_key_vault.domain_kv_dev.id
  tenant_id    = data.azurerm_client_config.current.tenant_id
  object_id    = module.DEV-CRUSC8-TLS-CERT-SERVICE-CONN.service_principal_object_id
...
# PROD
#
module "PROD-CRUSC8-TLS-CERT-SERVICE-CONN" {
  providers = {
...
resource "azurerm_key_vault_access_policy" "PROD-CRUSC8-TLS-CERT-SERVICE-CONN_kv_access_policy" {
  provider     = azurerm.prod
  key_vault_id = data.azurerm_key_vault.domain_kv_prod.id
  tenant_id    = data.azurerm_client_config.current.tenant_id
  object_id    = module.PROD-CRUSC8-TLS-CERT-SERVICE-CONN.service_principal_object_id
