...
resource "azuredevops_serviceendpoint_kubernetes" "aks_uat" {
  ...
  apiserver_url         = module.secrets.values["pagopa-u-weu-uat-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.secrets.values["pagopa-u-weu-uat-aks-azure-devops-sa-token"].value
    ca_cert = module.secrets.values["pagopa-u-weu-uat-aks-azure-devops-sa-cacrt"].value
  }
}
