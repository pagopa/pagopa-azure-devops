variable "pagopa-nodo-service-cron" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "pagopa-nodo4-nodo-dei-pagamenti"
      branch_name     = "refs/heads/develop"
      pipelines_path  = ".devops-cron"
...
locals {
  # global vars
  pagopa-nodo-service-cron-variables = {
    cache_version_id = "v1"
    default_branch   = var.pagopa-nodo-service-cron.repository.branch_name
  }
  # global secrets
  pagopa-nodo-service-cron-variables_secret = {
  ...
  # deploy vars
  pagopa-nodo-service-cron-variables_deploy = {
    ...
    # acr section
    image_repository_name                          = replace(var.pagopa-nodo-service-cron.repository.name, "-", "")
    container-registry-service-cron-connection-dev = data.terraform_remote_state.app.outputs.service_endpoint_azure_devops_acr_aks_dev_id
  ...
  # deploy secrets
  pagopa-nodo-service-cron-variables_secret_deploy = {
...
module "pagopa-nodo-service-cron_deploy" {
  ...
  count  = var.pagopa-nodo-service-cron.pipeline.enable_deploy == true ? 1 : 0
  ...
  variables = merge(
    local.pagopa-nodo-service-cron-variables,
    local.pagopa-nodo-service-cron-variables_deploy,
  )

  variables_secret = merge(
    local.pagopa-nodo-service-cron-variables_secret,
    local.pagopa-nodo-service-cron-variables_secret_deploy,
  )
