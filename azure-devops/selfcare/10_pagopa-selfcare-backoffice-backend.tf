variable "pagopa-selfcare-backoffice-backend" {
  default = {
    repository = {
      organization    = "pagopa"
...
locals {
  # global vars
  pagopa-selfcare-backoffice-backend-variables = {
    cache_version_id                 = "v1"
    default_branch                   = var.pagopa-selfcare-backoffice-backend.repository.branch_name
    settings_xml_ro_secure_file_name = "settings-ro.xml"
  }

  # global secrets
  pagopa-selfcare-backoffice-backend-variables_secret = {

  }

  # code_review vars
  pagopa-selfcare-backoffice-backend-variables_code_review = {
    danger_github_api_token = "skip"
    sonarcloud_service_conn = var.pagopa-selfcare-backoffice-backend.pipeline.sonarcloud.service_connection
    sonarcloud_org          = var.pagopa-selfcare-backoffice-backend.pipeline.sonarcloud.org
    sonarcloud_project_key  = var.pagopa-selfcare-backoffice-backend.pipeline.sonarcloud.project_key
    sonarcloud_project_name = var.pagopa-selfcare-backoffice-backend.pipeline.sonarcloud.project_name
  }

  # code_review secrets
  pagopa-selfcare-backoffice-backend-variables_secret_code_review = {
  }

  # deploy vars
  pagopa-selfcare-backoffice-backend-variables_deploy = {
  ...
  # deploy secrets
  pagopa-selfcare-backoffice-backend-variables_secret_deploy = {

  }
}

module "pagopa-selfcare-backoffice-backend_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v2.2.0"
  count  = var.pagopa-selfcare-backoffice-backend.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = data.azuredevops_project.project.id
  repository                   = var.pagopa-selfcare-backoffice-backend.repository
  github_service_connection_id = data.terraform_remote_state.app.outputs.service_endpoint_azure_devops_github_pr_id
  path                         = "${local.domain}\\pagopa-selfcare-backoffice-backend"

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.pagopa-selfcare-backoffice-backend-variables,
    local.pagopa-selfcare-backoffice-backend-variables_code_review,
  )

  variables_secret = merge(
    local.pagopa-selfcare-backoffice-backend-variables_secret,
    local.pagopa-selfcare-backoffice-backend-variables_secret_code_review,
  )
...
module "pagopa-selfcare-backoffice-backend_deploy" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_deploy?ref=v2.2.0"
  count  = var.pagopa-selfcare-backoffice-backend.pipeline.enable_deploy == true ? 1 : 0
  ...
  variables = merge(
    local.pagopa-selfcare-backoffice-backend-variables,
    local.pagopa-selfcare-backoffice-backend-variables_deploy,
  )

  variables_secret = merge(
    local.pagopa-selfcare-backoffice-backend-variables_secret,
    local.pagopa-selfcare-backoffice-backend-variables_secret_deploy,
  )
