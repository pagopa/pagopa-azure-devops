variable "tlscert-prf-api-prf-platform-pagopa-it" {
  default = {
    ...
    pipeline = {
      ...
      dns_record_name         = "api"
      dns_zone_name           = "prf.platform.pagopa.it"
...
locals {
  tlscert-prf-api-prf-platform-pagopa-it = {
...
module "tlscert-prf-api-prf-platform-pagopa-it-cert_az" {
  providers = {
    azurerm = azurerm.uat
  }
  ...
  name       = "${var.tlscert-prf-api-prf-platform-pagopa-it.pipeline.dns_record_name}.${var.tlscert-prf-api-prf-platform-pagopa-it.pipeline.dns_zone_name}"
  #tfsec:ignore:GEN003
  ...
  dns_zone_resource_group = var.tlscert-prf-api-prf-platform-pagopa-it.pipeline.dns_zone_resource_group
  tenant_id               = local.tlscert-prf-api-prf-platform-pagopa-it.tenant_id
  subscription_name       = local.tlscert-prf-api-prf-platform-pagopa-it.subscription_name
  subscription_id         = local.tlscert-prf-api-prf-platform-pagopa-it.subscription_id
  ...
  variables = merge(
    var.tlscert-prf-api-prf-platform-pagopa-it.pipeline.variables,
    local.tlscert-prf-api-prf-platform-pagopa-it-variables,
  )
