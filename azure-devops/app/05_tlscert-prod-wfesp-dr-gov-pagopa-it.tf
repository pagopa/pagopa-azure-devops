variable "tlscert-prod-wfesp-dr-pagopa-gov-it" {
  default = {
    repository = {
      organization   = "pagopa"
...
locals {
  tlscert-prod-wfesp-dr-pagopa-gov-it = {
    tenant_id         = data.azurerm_client_config.current.tenant_id
    subscription_name = "ORG"
    subscription_id   = module.secrets.values["ORG-SUBSCRIPTION-ID"].value
  }
  tlscert-prod-wfesp-dr-pagopa-gov-it-variables = {
    KEY_VAULT_SERVICE_CONNECTION = module.PROD-TLS-CERT-SERVICE-CONN.service_endpoint_name
  }
  tlscert-prod-wfesp-dr-pagopa-gov-it-variables_secret = {
  }
}

module "tlscert-prod-wfesp-dr-pagopa-gov-it-cert_az" {
  providers = {
  ...
  repository = var.tlscert-prod-wfesp-dr-pagopa-gov-it.repository
  name       = "${var.tlscert-prod-wfesp-dr-pagopa-gov-it.pipeline.dns_record_name}.${var.tlscert-prod-wfesp-dr-pagopa-gov-it.pipeline.dns_zone_name}"
  #tfsec:ignore:GEN003
  ...
  variables = merge(
    var.tlscert-prod-wfesp-dr-pagopa-gov-it.pipeline.variables,
    local.tlscert-prod-wfesp-dr-pagopa-gov-it-variables,
  )

  variables_secret = merge(
    var.tlscert-prod-wfesp-dr-pagopa-gov-it.pipeline.variables_secret,
    local.tlscert-prod-wfesp-dr-pagopa-gov-it-variables_secret,
  )
