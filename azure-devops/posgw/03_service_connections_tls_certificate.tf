...
# DEV
#
module "DEV-POS-GW-TLS-CERT-SERVICE-CONN" {
...
resource "azurerm_key_vault_access_policy" "DEV-POS-GW-TLS-CERT-SERVICE-CONN_kv_access_policy" {
  provider     = azurerm.dev
  key_vault_id = data.azurerm_key_vault.domain_kv_dev.id
  tenant_id    = data.azurerm_client_config.current.tenant_id
  object_id    = module.DEV-POS-GW-TLS-CERT-SERVICE-CONN.service_principal_object_id
