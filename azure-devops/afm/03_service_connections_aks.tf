...
resource "azuredevops_serviceendpoint_kubernetes" "aks_uat" {
  ...
  apiserver_url         = module.afm_uat_secrets.values["pagopa-u-weu-uat-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.afm_uat_secrets.values["pagopa-u-weu-uat-aks-azure-devops-sa-token"].value
    ca_cert = module.afm_uat_secrets.values["pagopa-u-weu-uat-aks-azure-devops-sa-cacrt"].value
  }
}
...
resource "azuredevops_serviceendpoint_kubernetes" "aks_prod" {
  ...
  apiserver_url         = module.afm_prod_secrets.values["pagopa-p-weu-prod-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.afm_prod_secrets.values["pagopa-p-weu-prod-aks-azure-devops-sa-token"].value
    ca_cert = module.afm_prod_secrets.values["pagopa-p-weu-prod-aks-azure-devops-sa-cacrt"].value
  }
}
