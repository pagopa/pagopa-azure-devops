resource "azuredevops_serviceendpoint_kubernetes" "aks_dev" {
  ...
  apiserver_url         = module.payopt_dev_secrets.values["pagopa-d-itn-dev-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.payopt_dev_secrets.values["pagopa-d-itn-dev-aks-azure-devops-sa-token"].value
    ca_cert = module.payopt_dev_secrets.values["pagopa-d-itn-dev-aks-azure-devops-sa-cacrt"].value
  }
}
...
resource "azuredevops_serviceendpoint_kubernetes" "aks_uat" {
  ...
  apiserver_url         = module.payopt_uat_secrets.values["pagopa-u-itn-uat-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.payopt_uat_secrets.values["pagopa-u-itn-uat-aks-azure-devops-sa-token"].value
    ca_cert = module.payopt_uat_secrets.values["pagopa-u-itn-uat-aks-azure-devops-sa-cacrt"].value
  }
}
