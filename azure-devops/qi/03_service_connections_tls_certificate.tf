...
# DEV
#
module "DEV-QI-TLS-CERT-SERVICE-CONN" {
...
resource "azurerm_key_vault_access_policy" "DEV-QI-TLS-CERT-SERVICE-CONN_kv_access_policy" {
  provider     = azurerm.dev
  key_vault_id = data.azurerm_key_vault.domain_kv_dev.id
  tenant_id    = module.secrets.values["TENANTID"].value
  object_id    = module.DEV-QI-TLS-CERT-SERVICE-CONN.service_principal_object_id
...
# UAT
#
module "UAT-QI-TLS-CERT-SERVICE-CONN" {
  providers = {
    azurerm = azurerm.uat
  }
...
resource "azurerm_key_vault_access_policy" "UAT-QI-TLS-CERT-SERVICE-CONN_kv_access_policy" {
  provider     = azurerm.uat
  key_vault_id = data.azurerm_key_vault.domain_kv_uat.id
  tenant_id    = module.secrets.values["TENANTID"].value
  object_id    = module.UAT-QI-TLS-CERT-SERVICE-CONN.service_principal_object_id
...
# PROD
#
module "PROD-QI-TLS-CERT-SERVICE-CONN" {
  providers = {
...
resource "azurerm_key_vault_access_policy" "PROD-QI-TLS-CERT-SERVICE-CONN_kv_access_policy" {
  provider     = azurerm.prod
  key_vault_id = data.azurerm_key_vault.domain_kv_prod.id
  tenant_id    = module.secrets.values["TENANTID"].value
  object_id    = module.PROD-QI-TLS-CERT-SERVICE-CONN.service_principal_object_id
