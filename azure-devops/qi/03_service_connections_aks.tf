resource "azuredevops_serviceendpoint_kubernetes" "aks_dev" {
  ...
  apiserver_url         = module.qi_dev_secrets.values["pagopa-d-weu-dev-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.qi_dev_secrets.values["pagopa-d-weu-dev-aks-azure-devops-sa-token"].value
    ca_cert = module.qi_dev_secrets.values["pagopa-d-weu-dev-aks-azure-devops-sa-cacrt"].value
  }
}
...
resource "azuredevops_serviceendpoint_kubernetes" "aks_prod" {
  ...
  apiserver_url         = module.qi_prod_secrets.values["pagopa-p-weu-prod-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.qi_prod_secrets.values["pagopa-p-weu-prod-aks-azure-devops-sa-token"].value
    ca_cert = module.qi_prod_secrets.values["pagopa-p-weu-prod-aks-azure-devops-sa-cacrt"].value
  }
}
