variable "tlscert-prod-kibana-platform-pagopa-it" {
  default = {
    repository = {
      organization   = "pagopa"
...
locals {
  tlscert-prod-kibana-platform-pagopa-it = {
    tenant_id         = data.azurerm_client_config.current.tenant_id
    subscription_name = "PROD-PAGOPA"
    subscription_id   = data.azurerm_subscriptions.prod.subscriptions[0].subscription_id
  }
  tlscert-prod-kibana-platform-pagopa-it-variables = {
    KEY_VAULT_SERVICE_CONNECTION = module.PROD-TLS-CERT-SERVICE-CONN.service_endpoint_name
  }
  tlscert-prod-kibana-platform-pagopa-it-variables_secret = {
  }
}

module "tlscert-prod-kibana-platform-pagopa-it-cert_az" {
  ...
  dns_record_name                      = var.tlscert-prod-kibana-platform-pagopa-it.pipeline.dns_record_name
  dns_zone_name                        = var.tlscert-prod-kibana-platform-pagopa-it.pipeline.dns_zone_name
  dns_zone_resource_group              = var.tlscert-prod-kibana-platform-pagopa-it.pipeline.dns_zone_resource_group
  tenant_id                            = local.tlscert-prod-kibana-platform-pagopa-it.tenant_id
  ...
  variables = merge(
    var.tlscert-prod-kibana-platform-pagopa-it.pipeline.variables,
    local.tlscert-prod-kibana-platform-pagopa-it-variables,
  )

  variables_secret = merge(
    var.tlscert-prod-kibana-platform-pagopa-it.pipeline.variables_secret,
    local.tlscert-prod-kibana-platform-pagopa-it-variables_secret,
  )
